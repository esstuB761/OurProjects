    ...
    public void onUpdateReceived(Update update) {
        if (update.hasMessage()) {
            if (update.getMessage().hasText()) {
                ...
                } else if (lowerText.equals( BotCommand.ФИО.getCommandText())) {
                    sendResponse(update.getMessage().getChatId(), "Привет Таргашина Екатерина студент Б761! "+ update.getMessage().getFrom().getUserName());
