...
public class Main {
            ...
            switch (choice) {
                ...
                case 6:
                    try {
                        ...
                        log.info("Записи выгружены из БД");
                        saveConsumers();
                ...
                case 7:
                    try {
                        log.info("Записи загружены в БД");
    ...
    public static void connect() throws Exception {
        ...
        if (conn == null) {
            log.info("Подключиться к БД не удалось");
            throw new Exception("Подключиться к БД не удалось");
        } else {
    ...
    private static void generatePdf(PDDocument pdDoc, BaseTable table, PDFont font) throws IOException {
        ...
        table.draw();
        img.draw(pdDoc, pcos, 250, 150);
        drawQR(pdDoc, pcos, 275 , 75);
        cos.close();
        try {
    ...
    private static void drawQR(PDDocument pdDoc, PageContentStreamOptimized stream, float x, float y) throws IOException {
        String stringBuf = "https://pestotesto.ru/";
    ...
    private static byte[] generateQR(String content) {
        ...
        try (arrayOutputStream) {
            BitMatrix bitMatrix = qrCodeWriter.encode(content, BarcodeFormat.QR_CODE, 50, 50, hints);
