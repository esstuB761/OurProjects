    ...
    public void onUpdateReceived(Update update) {
        if (update.hasMessage()) {
            if (update.getMessage().hasText()) {
                ...
                } else if (lowerText.equals( BotCommand.НИКНЕЙМ.getCommandText())) {
                    sendResponse(update.getMessage().getChatId(), "Привет, студент! Ваш username - "+ update.getMessage().getFrom().getUserName());
